        ...
        : IEnumerable<GeckoNode>
    {
        ...
        public GeckoNode this[int index]
        {
            get
            {
                ...
                return ExtensionMethods.Wrap(_map.Item((uint)index),
                         GeckoNode.Create);
        ...
        public GeckoNode this[string name]
        {
            get
            {
                return ExtensionMethods.Wrap(nsString.Pass(_map.GetNamedItem, name),
                    GeckoNode.Create); // return nsString.Pass(_map.GetNamedItem, name).Wrap(GeckoNode.Create);
            }
        }
        ...
        public GeckoNode this[string namespaceUri, string localName]
        {
            get
            {
                return ExtensionMethods.Wrap(nsString.Pass(_map.GetNamedItemNS, namespaceUri, localName),
                    GeckoNode.Create); // nsString.Pass( _map.GetNamedItemNS, namespaceUri, localName ).Wrap( GeckoNode.Create ); 
            }
        }

        public GeckoNode RemoveNamedItem(string name)
        {
            return ExtensionMethods.Wrap(nsString.Pass(_map.RemoveNamedItem, name),
                    GeckoNode.Create); // nsString.Pass(_map.RemoveNamedItem, name)
