...
namespace Gecko
{
		...
		: GeckoNode
	{
		...
		/// <returns></returns>
		public IGeckoArray<GeckoElement> GetElementsByTagName(string tagName)
		{
			if (string.IsNullOrEmpty(tagName))
				return null;

            return ExtensionMethods.Wrap(nsString.Pass(_domElement.GetElementsByTagName, tagName),
                   x => new DomNodeList<GeckoElement, nsIDOMElement>(x, CreateDomElementWrapper));
            //return nsString.Pass( _domElement.GetElementsByTagName,tagName )
            //    .Wrap( x => new DomNodeList<GeckoElement, nsIDOMElement>( x, CreateDomElementWrapper ) );
		}
		...
		public IGeckoArray<GeckoElement> GetElementsByTagNameNS(string namespaceURI, string localName)
		{
            ...
            return ExtensionMethods.Wrap(nsString.Pass<nsIDOMNodeList>(_domElement.GetElementsByTagNameNS, namespaceURI, localName),
                    x => new DomNodeList<GeckoElement, nsIDOMElement>(x, CreateDomElementWrapper));
            //return nsString.Pass<nsIDOMNodeList>( _domElement.GetElementsByTagNameNS, namespaceURI, localName )
            //               .Wrap( x => new DomNodeList<GeckoElement, nsIDOMElement>( x, CreateDomElementWrapper ) );
		}
