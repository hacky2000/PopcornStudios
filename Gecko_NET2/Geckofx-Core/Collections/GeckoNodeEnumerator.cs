...
namespace Gecko.Collections
{
		...
		where TWrapper:class
	{
		...
		private BaseTypes.Func<TGeckoNode, TWrapper> _translator;

		internal GeckoNodeEnumerator( nsIDOMNodeList list, BaseTypes.Func<TGeckoNode, TWrapper> translator )
			: this( new Wrapper1( list ), translator )
		{
		}


		public GeckoNodeEnumerator(nsIDOMHTMLCollection collection, BaseTypes.Func<TGeckoNode, TWrapper> translator)
			: this(new Wrapper2(collection), translator)
		{

		}

		private GeckoNodeEnumerator( Wrapper wrapper, BaseTypes.Func<TGeckoNode, TWrapper> translator )
		{
			_wrapper = wrapper;
