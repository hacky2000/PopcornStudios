...
namespace Gecko.Services
{
    public static class WindowWatcher
    {
        ...
        public static GeckoWindow ActiveWindow
        {
            get
            {
                return ExtensionMethods.Wrap(_watcher.Instance.GetActiveWindowAttribute(), GeckoWindow.Create);
                // _watcher.Instance.GetActiveWindowAttribute().Wrap( GeckoWindow.Create );
            }
